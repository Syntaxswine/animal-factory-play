import {syncPeople,advancePeople,efficiency,findPath,perimeter} from './people.js';
import {newEstate,advanceEstate,expandEstate,setRationShare} from './estate.js';
import {advanceServices} from './services.js';
...
export const TYPES = {
  ...
  pavilion:{name:'Private pavilion',worker:'donkey',color:'#a75736',group:'estate',sprite:'pavilion.svg'},
  well:{name:'Village well',service:'water',color:'#8aaca3',group:'workers',sprite:'well.svg'},
  clinic:{name:'Clinic',service:'health',worker:'goat',color:'#d3c5a6',group:'workers',sprite:'clinic.svg'},
  kitchen:{name:'Communal kitchen',service:'food',worker:'hen',accepts:['ration'],capacity:32,color:'#a3774d',group:'workers',sprite:'kitchen.svg'},
  residence:{name:'Workers’ cottages',residents:3,color:'#b2986a',group:'workers',sprite:'residence.svg'},
...
export function recipeText(type){const t=TYPES[type];if(type==='pavilion')return 'Reserves 3 × 3 tiles and one resident attendant for the pigs. One staffed pavilion permits estate level II; two permit level III. Expand using retained bread, alcohol and cakes.';if(type==='well')return 'Supplies water to homes within 12 walking tiles. Keep a path open; no staffing required.';if(type==='clinic')return 'One resident runs the clinic during work hours. Homes within 12 walking tiles recover health faster. Needs a worker physically present.';if(type==='kitchen')return 'Stores 32 rations. With a cook present, one ration restores 40 food instead of 25. Outside opening hours it remains a normal ration pickup.';if(type==='residence')return 'Homes for 3 residents. Residents fill reachable jobs within 36 walking tiles. Demolition makes these residents leave; their jobs become vacant.';if(type==='depot')return 'Stores 32 rations. Feed the green input by conveyor; workers collect meals from the open ground around the depot.';if(t.accepts)return 'Returns the selected share of bread and cake as worker rations (20% by default). Send the green outlet to a ration depot or an accessible belt end. Deliveries continue when rations back up. Alcohol produces no rations.';const from=Object.entries(t.recipe||{}).map(([g,n])=>`${n} ${GOODS[g].name.toLowerCase()}`).join(' + ');return `${from?from+' → ':'Produces '}1 ${GOODS[t.out].name.toLowerCase()} / ${t.time}s`;}
export class Farm{
  constructor(){this.buildings=[];this.belts={};this.roads={};this.stats={wheat:0,flour:0,bread:0,alcohol:0,cake:0};this.produced={};this.delivered={bread:0,alcohol:0,cake:0};this.clock=0;this.stepClock=0;this.nextId=1;this.plan='bread';this.people=[];this.rationsEaten=0;this.estate=newEstate();this.rationShare=20;}
  inside(x,y){return x>=0&&y>=0&&x<COLS&&y<ROWS;}
  at(x,y){return this.buildings.find(b=>x>=b.x&&x<b.x+3&&y>=b.y&&y<b.y+3);}
  canPlace(x,y){for(let j=y;j<y+3;j++)for(let i=x;i<x+3;i++)if(!this.inside(i,j)||this.at(i,j)||this.belts[key(i,j)]||this.roads[key(i,j)])return false;return true;}
  ...
  layRoad(cells){if(!this.canLay(cells))return false;for(const c of cells)this.roads[key(c.x,c.y)]={x:c.x,y:c.y};syncPeople(this,TYPES);return true;}
  remove(cells){let n=0;for(const c of cells){if(this.roads[key(c.x,c.y)]){delete this.roads[key(c.x,c.y)];n++;}const b=this.at(c.x,c.y);if(b&&b.type!=='house'){this.buildings=this.buildings.filter(a=>a!==b);n++;}if(this.belts[key(c.x,c.y)]){delete this.belts[key(c.x,c.y)];n++;}}syncPeople(this,TYPES);return n;}
  expandEstate(){return expandEstate(this);}
  setRationShare(value){return setRationShare(this,value);}
  ...
  advance(dt){this.clock+=dt;for(const b of this.buildings){const t=TYPES[b.type];if(b.type==='house'){if((b.rationReserve||0)>=5&&b.output<4){b.rationReserve-=5;b.output++;this.produced.ration=(this.produced.ration||0)+1;}continue;}if(!t.out||b.output>=4||missingInputs(b).length)continue;b.progress+=dt*efficiency(this,b);if(b.progress>=t.time){b.progress-=t.time;for(const [g,n]of Object.entries(t.recipe||{}))b.inventory[g]-=n;b.output++;this.produced[t.out]=(this.produced[t.out]||0)+1;if(t.out==='wheat'||t.out==='flour')this.stats[t.out]++;}}
    this.stepClock+=dt;while(this.stepClock>=.3){this.stepClock-=.3;this.move();}advancePeople(this,dt);advanceServices(this,dt);advanceEstate(this,dt);}
  move(){
    ...
    for(const b of Object.values(this.belts)){if(!b.item)continue;const[dx,dy]=DIRS[b.dir],nx=b.x+dx,ny=b.y+dy,nk=key(nx,ny),target=this.belts[nk];
      ...
      if(building.type==='house'){const food=b.item==='bread'||b.item==='cake';this.delivered[b.item]++;this.stats[b.item]++;if(food)building.rationReserve=(building.rationReserve||0)+this.rationShare/20;this.estate.stock[b.item]+=(food?1-this.rationShare/100:1);}else building.inventory[b.item]=(building.inventory[b.item]||0)+1;b.item=null;
    }
...
export function starter(connected=false,plan='bread'){const f=new Farm();f.plan=plan;for(const[type,x,y,dir=0]of PLANS[plan].buildings)f.place(type,x,y,dir);const count=Math.ceil(f.buildings.filter(b=>TYPES[b.type].worker&&b.type!=='house').length/3);for(const [x,y] of [[3,16],[10,16],[3,20]].slice(0,count))f.place('residence',x,y);if(connected)connectExample(f);return f;}
...
export function connectExample(f){const plan=PLANS[f.plan];if(!plan.buildings.every(([type,x,y,dir=0])=>f.buildings.some(b=>b.type===type&&b.x===x&&b.y===y&&b.dir===dir)))return false;if(!f.lay(exampleCells(f.plan)))return false;const streets=[];for(const p of f.people.filter(p=>!p.pig)){const home=f.buildings.find(b=>b.id===p.home);for(const target of f.buildings.filter(b=>b.id===p.job||b.type==='depot')){const path=findPath(f,perimeter(home),perimeter(target));if(path)streets.push(...path);}}f.layRoad(streets);return true;}

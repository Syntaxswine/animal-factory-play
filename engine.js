...
export const TYPES = {
  ...
  house:{name:'Farmhouse',accepts:['bread','alcohol','cake'],color:'#963f35',worker:'pig-director'},
};
...
export function recipeText(type){const t=TYPES[type];if(t.accepts)return 'Receives bread, alcohol, and cake at three separate hatches.';const from=Object.entries(t.recipe||{}).map(([g,n])=>`${n} ${GOODS[g].name.toLowerCase()}`).join(' + ');return `${from?from+' → ':'Produces '}1 ${GOODS[t.out].name.toLowerCase()} / ${t.time}s`;}
export class Farm{
  constructor(){this.buildings=[];this.belts={};this.stats={wheat:0,flour:0,bread:0,alcohol:0,cake:0};this.produced={};this.delivered={bread:0,alcohol:0,cake:0};this.clock=0;this.stepClock=0;this.nextId=1;this.plan='bread';}
  ...
  place(type,x,y,dir=0){if(!TYPES[type]||!this.canPlace(x,y))return false;const b={id:this.nextId++,type,x,y,dir,inventory:{},output:0,progress:0};this.buildings.push(b);return b;}
  canLay(cells){return cells.every(c=>this.inside(c.x,c.y)&&!this.at(c.x,c.y));}
  lay(cells){if(!this.canLay(cells))return false;for(const c of cells)this.belts[key(c.x,c.y)]={...c,item:this.belts[key(c.x,c.y)]?.item??null};return true;}
  ...
  advance(dt){this.clock+=dt;for(const b of this.buildings){const t=TYPES[b.type];if(!t.out||b.output>=4||missingInputs(b).length)continue;b.progress+=dt;if(b.progress>=t.time){b.progress-=t.time;for(const [g,n]of Object.entries(t.recipe||{}))b.inventory[g]-=n;b.output++;this.produced[t.out]=(this.produced[t.out]||0)+1;if(t.out==='wheat'||t.out==='flour')this.stats[t.out]++;}}
  ...
  move(){
    ...
    for(const b of Object.values(this.belts)){if(!b.item)continue;const[dx,dy]=DIRS[b.dir],nx=b.x+dx,ny=b.y+dy,nk=key(nx,ny),target=this.belts[nk];
      ...
      if(building.type==='house'){this.delivered[b.item]++;this.stats[b.item]++;}else building.inventory[b.item]=(building.inventory[b.item]||0)+1;b.item=null;
    }
...
export function exampleCells(plan){return PLANS[plan].routes.flatMap(route=>route.slice(1).flatMap((p,i)=>line({x:route[i][0],y:route[i][1]},{x:p[0],y:p[1]})));}
